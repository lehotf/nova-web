...
$titulo     = "ADICIONAR geoMask";
$explicacao = 'Máscara de IP por localização';

define("LOG_FOLDER", $_SERVER['DOCUMENT_ROOT'] . '/log/mask/novas');
...
$conteudo = '<div id="lista_mask">';

$time = time();
foreach ($lista as $mask) {    
    $conteudo .= '<p>' . $mask . '</p><a class="mask_ok" href="/comum/php/sistema/acesso/add_geomask.php?mask=' . $mask . '&action=ok&time='.$time.'">OK</a><a class="mask_del"href="/comum/php/sistema/acesso/add_geomask.php?mask=' . $mask . '&action=del&time='.$time.'">DEL</a>';
