...
// faixa que passa a ter alíquota final de 15% para fundo de longo prazo.
$baldes = [];
for ($i = 1; $i <= 25; $i++) {
    $baldes[$i] = [
        'investido' => 0,
...
foreach ($baldes as $age => $b) {
    ...
    elseif ($age <= 12) $aliquota = 0.20;
    elseif ($age <= 24) $aliquota = 0.175;
    else $aliquota = 0.15;
    
    $rendimento_recente = max(0, $b['saldo'] - $b['base_atual']);
    $imposto_recente = $rendimento_recente * $aliquota;
    
    $diferenca_aliquota = max(0, $aliquota - 0.15);
