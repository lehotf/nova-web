(function () {
    ...
    function renderCalculator(articleBody) {
        ...
        wrapper.innerHTML = '' +
            ...
            '  <div class="calc-rf__result-main">Valor líquido projetado ao final do período no fundo de renda fixa de curto prazo <span class="calc-rf__result-value">-</span></div>' +
            ...
            window.send({
                ...
                f: function (response) {
                    ...
                    var htmlRelatorio = '<div style="font-size: 16px; line-height: 1.6; margin-top: 30px;">' +
                        ...
                        '<p>Para fundos de renda fixa de curto prazo sujeitos a come-cotas, a legislação prevê antecipação semestral de IR, normalmente no último dia útil de maio e novembro. Nesta simulação mensal, essa cobrança foi aproximada nesses dois meses e calculada à alíquota de <strong>20%</strong> sobre o rendimento acumulado desde o último evento de come-cotas. A quantidade estimada de cotas perdidas no período foi equivalente a <strong>' + currencyFormatter.format(result.total_descontado_come_cotas) + '</strong>.</p>' +
                        '<p>No resgate, pode existir um imposto complementar de <strong>' + currencyFormatter.format(result.imposto_resgate) + '</strong>. Esse valor representa a diferença entre a alíquota final regressiva aplicável ao prazo do aporte e o que já foi antecipado pelo come-cotas. Nesta calculadora, essa tabela foi aproximada por meses: até 6 meses, 22,5%; acima de 6 meses, 20%. A alíquota média efetiva de IR neste exemplo foi de <strong>' + numberFormatter.format(aliquotaMediaIr) + '%</strong> sobre o ganho bruto estimado do fundo.</p>' +
                    ...
                    if (result.relatorio && result.relatorio.length > 0) {
                        ...
                        if (hasComeCotas) {
                            htmlRelatorio += '<h4 style="margin-top: 30px;">Detalhamento da Cobrança de Come-cotas</h4>' +
                                ...
                                '<thead>' +
                                '<tr><th style="padding: 8px; border-bottom: 1px solid #ccc;">Mês/Ano</th><th style="padding: 8px; border-bottom: 1px solid #ccc;">Come-cotas (20%)</th><th style="padding: 8px; border-bottom: 1px solid #ccc;">Saldo Pós-Retenção</th></tr>' +
                                '</thead><tbody>';
