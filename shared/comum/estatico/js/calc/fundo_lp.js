            ...
            window.send({
                ...
                f: function (response) {
                    ...
                    var htmlRelatorio = '<div class="calc-rf__report-content">' +
                        ...
                        '<p>Para fundos de renda fixa de longo prazo sujeitos a come-cotas, a legislação prevê antecipação semestral de IR, normalmente no último dia útil de maio e novembro. Nesta simulação mensal, essa cobrança foi aproximada nesses dois meses e calculada à alíquota de <strong>15%</strong> sobre o rendimento acumulado desde o último evento de come-cotas. A quantidade estimada de cotas perdidas no período foi equivalente a ' + comeCotasFmt + '.</p>' +
                        '<p>No resgate, calcula-se imposto complementar de ' + impostoFmt + '. Esse valor representa a diferença entre a alíquota final regressiva aplicável ao prazo do aporte e o que já foi antecipado pelo come-cotas. Nesta calculadora, essa tabela foi aproximada por meses: até 6 meses, 22,5%; de 7 a 12 meses, 20%; de 13 a 24 meses, 17,5%; acima de 24 meses, 15%. A alíquota média efetiva de IR neste exemplo foi de <strong>' + numberFormatter.format(aliquotaMediaIr) + '%</strong> sobre o ganho bruto estimado do fundo.</p>' +
                    ...
                    if (result.relatorio && result.relatorio.length > 0) {
                        ...
                        if (hasComeCotas) {
                            htmlRelatorio += '<h4 class="calc-rf__mt-30">Detalhamento da Cobrança de Come-cotas</h4>' +
                                ...
                                '<thead>' +
                                '<tr><th>Mês/Ano</th><th>Come-cotas (15%)</th><th>Saldo Pós-Retenção</th></tr>' +
                                '</thead><tbody>';
