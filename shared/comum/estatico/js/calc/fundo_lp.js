(function () {
    var WIDGET_ID = 'calc-fundo-lp-widget';
    var RESULT_ID = 'calc-fundo-lp-resultado';
    var ENDPOINT_ACTION = 'comum/calc/fundo_lp';
    ...
    function renderCalculator(articleBody) {
        ...
        wrapper.innerHTML = '' +
            ...
            '    </label>' +
            '  </div>' +
            '  <p class="calc-rf__disclaimer" style="margin: 8px 0 0; font-size: 13px; line-height: 1.5; opacity: 0.85;">Simulação educacional para <strong>fundo de renda fixa de longo prazo</strong>, com aproximação mensal do IR regressivo, come-cotas semestral de 15% e inflação projetada pela repetição da última taxa mensal de IGP-M disponível.</p>' +
            ...
            '  <div class="calc-rf__result-main">Valor líquido projetado ao final do período no fundo de renda fixa de longo prazo <span class="calc-rf__result-value">-</span></div>' +
            ...
            '    <div class="calc-rf__card" style="opacity: 0.55;"><span class="calc-rf__card-label">Come-cotas (já retido)</span><span class="calc-rf__card-value" data-role="come_cotas">-</span></div>' +
            '  </div>' +
            ...
            window.send({
                ...
                f: function (response) {
                    ...
                    impostoResgateValue.textContent = currencyFormatter.format(result.imposto_resgate || 0);
                        ...
                        ', esta estimativa considera um fundo de renda fixa de longo prazo. A "Rentabilidade do Fundo" já reflete a perda de cotas e o custo de oportunidade do come-cotas semestral de 15%.';
                    ...
                    var htmlRelatorio = '<div style="font-size: 16px; line-height: 1.6; margin-top: 30px;">' +
                        '<h3 style="margin-bottom: 15px; font-size: 20px;">Explicação Detalhada</h3>' +
                        '<p><strong>Importante:</strong> esta calculadora foi modelada para <strong>fundo de renda fixa de longo prazo</strong>. O imposto regressivo foi aproximado em meses, embora a legislação use contagem em dias, e a inflação foi projetada pela repetição da última taxa mensal de IGP-M disponível na base.</p>' +
                        ...
                        '<p>Este investimento lhe dará uma rentabilidade de <strong>' + currencyFormatter.format(result.juros_acumulados) + '</strong>; totalizando um valor final no fundo de <strong>' + currencyFormatter.format(saldoAntesDoResgate) + '</strong>. Esta rentabilidade representaria um ganho de <strong>' + numberFormatter.format(percentualGanhoRelativo) + '%</strong> em relação ao valor investido.</p>' +
                        '<p>Para fundos de renda fixa de longo prazo sujeitos a come-cotas, a legislação prevê antecipação semestral de IR, normalmente no último dia útil de maio e novembro. Nesta simulação mensal, essa cobrança foi aproximada nesses dois meses e calculada à alíquota de <strong>15%</strong> sobre o rendimento acumulado desde o último evento de come-cotas. A quantidade estimada de cotas perdidas no período foi equivalente a <strong>' + currencyFormatter.format(result.total_descontado_come_cotas) + '</strong>.</p>' +
                        '<p>No resgate, pode existir um imposto complementar de <strong>' + currencyFormatter.format(result.imposto_resgate) + '</strong>. Esse valor representa a diferença entre a alíquota final regressiva aplicável ao prazo do aporte e o que já foi antecipado pelo come-cotas. Nesta calculadora, essa tabela foi aproximada por meses: até 6 meses, 22,5%; de 7 a 12 meses, 20%; de 13 a 24 meses, 17,5%; acima de 24 meses, 15%. A alíquota média efetiva de IR neste exemplo foi de <strong>' + numberFormatter.format(aliquotaMediaIr) + '%</strong> sobre o ganho bruto estimado do fundo.</p>' +
                        '<p>Portanto, a rentabilidade líquida do investimento será de <strong>' + currencyFormatter.format(rentabilidadeLiquida) + '</strong>. Percentualmente falando, isto corresponde a <strong>' + numberFormatter.format(percentualRentabilidadeLiquida) + '%</strong> do valor investido. O valor líquido que você receberá na conta é de <strong>' + currencyFormatter.format(result.valor_liquido) + '</strong>.</p>' +
                        '<h4 style="margin-top: 25px; margin-bottom: 10px; font-size: 18px;">Considerando a Inflação:</h4>' +
                        ...
                        '<p>A rentabilidade real líquida, então, será de <strong>' + currencyFormatter.format(result.rentabilidade_real_liquida) + '</strong>, já considerando o valor líquido final e a projeção inflacionária adotada.</p>' +
                        '<p>Isto significa uma rentabilidade real líquida, considerando impostos e inflação, de <strong>' + numberFormatter.format(percentualRentabilidadeRealLiquida) + '%</strong> no período de <strong>' + result.numero_meses + ' meses</strong>.</p>' +
                        '</div>';
                    ...
                    if (result.relatorio && result.relatorio.length > 0) {
                        ...
                        if (hasComeCotas) {
                            htmlRelatorio += '<h4 style="margin-top: 30px;">Detalhamento da Cobrança de Come-cotas</h4>' +
                                '<div style="overflow-x: auto;">' +
                                '<table style="width: 100%; border-collapse: collapse; text-align: left; margin-top: 10px;">' +
                                '<thead>' +
                                '<tr><th style="padding: 8px; border-bottom: 1px solid #ccc;">Mês/Ano</th><th style="padding: 8px; border-bottom: 1px solid #ccc;">Come-cotas (15%)</th><th style="padding: 8px; border-bottom: 1px solid #ccc;">Saldo Pós-Retenção</th></tr>' +
                                '</thead><tbody>';
                            
                            result.relatorio.forEach(function (linha) {
                                if (linha.desconto_come_cotas > 0) {
                                    htmlRelatorio += '<tr>' +
                                        '<td style="padding: 8px; border-bottom: 1px solid #eee;">' + linha.mes_ano + '</td>' +
                                        '<td style="padding: 8px; border-bottom: 1px solid #eee; color: red; font-weight: bold;">-' + currencyFormatter.format(linha.desconto_come_cotas) + '</td>' +
                                        '<td style="padding: 8px; border-bottom: 1px solid #eee;">' + currencyFormatter.format(linha.saldo_final) + '</td>' +
                                        '</tr>';
                                }
                            });
